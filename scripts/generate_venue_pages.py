#!/usr/bin/env python3
"""Generate /venues/ (hub) + /venues/<city>/ pages from data/venues.json.

"Live music venues in <city>" — high-intent local SEO. Server-rendered, with
MusicVenue + ItemList JSON-LD per venue and Breadcrumb schema, and a two-way
internal-linking loop with /live/. Runs daily in the pipeline.
"""
import json, html, datetime
from pathlib import Path

REPO = Path(__file__).resolve().parent.parent
SRC = REPO / "data" / "venues.json"
VDIR = REPO / "venues"

def esc(s): return html.escape(str(s or ""), quote=True)

def fmt(iso):
    try: return datetime.date.fromisoformat(iso).strftime("%d %b")
    except Exception: return ""

SRC_LABEL = {"district": "District", "bookmyshow": "BookMyShow", "skillboxes": "Skillboxes",
             "paytm": "Paytm Insider", "thumpn": "thumpN", "highape": "HighApe"}

# Shared <head> style + nav, mirrors /live/ and /news/.
...
.cities{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;margin-top:26px}
.citycard{border:1px solid var(--border);border-radius:14px;padding:20px;background:var(--surface)}
.citycard h3{font-size:20px;font-weight:800;letter-spacing:-.3px}
.citycard p{font-size:13px;color:var(--muted);margin-top:4px}
.citycard .go{font-size:13px;font-weight:700;color:var(--accent);margin-top:10px;display:inline-block}
.venue{border:1px solid var(--border);border-radius:14px;padding:18px 20px;background:var(--surface);margin-bottom:12px}
.vtop{display:flex;justify-content:space-between;align-items:flex-start;gap:12px}
.venue h3{font-size:19px;font-weight:800;letter-spacing:-.3px;line-height:1.25}
.varea{font-size:13px;color:var(--muted);font-weight:600;margin-top:2px}
.vcount{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.5px;color:var(--accent);background:rgba(233,69,96,.12);padding:5px 10px;border-radius:20px;white-space:nowrap;flex-shrink:0}
.grow{border-top:1px solid var(--border);padding:9px 0 2px;margin-top:9px;display:flex;gap:12px;align-items:baseline;flex-wrap:wrap}
.gwhen{font-size:13px;font-weight:800;font-variant-numeric:tabular-nums;min-width:52px}
.gname{font-size:14px;color:#d4d4e2;flex:1;min-width:0}
.gtix{font-size:12px;font-weight:700;color:var(--accent)}
.gprice{font-size:12px;color:var(--muted)}
.more{font-size:12px;color:var(--muted);margin-top:8px}
h2.sec{font-size:13px;font-weight:800;text-transform:uppercase;letter-spacing:1.2px;color:var(--accent);margin:34px 0 4px}
...
nav.js-primnav a{font-size:22px!important;font-weight:700!important;padding:14px 30px!important;height:auto!important}}'''
...
HEADER = '''<header><div class="logo"><a href="/">◉ Indie Music India</a><span style="display:block;font-weight:400;font-size:11px;letter-spacing:0;color:var(--muted);margin-top:2px;">Free tools &amp; data for India's independent music scene</span></div></header>'''

BURGER = '''<button class="mnav-burger" aria-label="Menu" aria-expanded="false" onclick="var o=document.body.classList.toggle('mnav-open');this.setAttribute('aria-expanded',o)"><span></span><span></span><span></span></button>'''

SCRIPTJS = '''<script>document.addEventListener('click',function(e){var a=e.target.closest('nav.js-primnav a');if(a)document.body.classList.remove('mnav-open');});document.addEventListener('keydown',function(e){if(e.key==='Escape')document.body.classList.remove('mnav-open');});</script>'''
...
FOOT = f'''<footer>
  ...
  <div class="footer-links">
    <a href="/">Home</a><a href="/live/">Live</a><a href="/venues/">Venues</a><a href="/news/">News</a>
...
def venue_card(v):
    area = f'<div class="varea">{esc(v["area"])}, {esc(v["city"])}</div>' if v.get("area") else f'<div class="varea">{esc(v["city"])}</div>'
    rows = []
    for g in v["gigs"][:5]:
        when = fmt(g["date"]) or "TBA"
        price = f'<span class="gprice">{esc(g["price"])}</span>' if g.get("price") else ""
        src = SRC_LABEL.get(g.get("source", ""), g.get("source", "").title())
        tix = f'<a class="gtix" href="{esc(g["url"])}" target="_blank" rel="noopener">Tickets · {esc(src)} ↗</a>' if g.get("url") else ""
        rows.append(f'<div class="grow"><span class="gwhen">{when}</span><span class="gname">{esc(g["name"])}</span>{price}{tix}</div>')
    more = f'<div class="more">+ {v["shows"] - 5} more upcoming</div>' if v["shows"] > 5 else ""
    return f'''  <div class="venue">
    <div class="vtop">
      <div><h3>{esc(v["name"])}</h3>{area}</div>
      <span class="vcount">{v["shows"]} upcoming</span>
    </div>
{chr(10).join(rows)}
    {more}
  </div>'''
...
def venue_schema(v, city_name):
    addr = ", ".join([x for x in [v.get("area"), city_name] if x])
    return ('{"@type":"MusicVenue","name":%s,"address":{"@type":"PostalAddress","addressLocality":%s,"addressRegion":%s,"addressCountry":"IN"}}'
            % (json.dumps(v["name"]), json.dumps(v.get("area") or city_name), json.dumps(city_name)))


def build():
    data = json.load(open(SRC))
    cities = data["cities"]
    updated = fmt(data.get("generated_at", "")[:10]) or datetime.date.today().strftime("%d %b")

    # ---- city hub /venues/ ----
    order = sorted(cities.items(), key=lambda kv: -kv[1]["venue_count"])
    cards = "\n".join(
        f'''  <a class="citycard" href="/venues/{esc(slug)}/">
    <h3>{esc(c["name"])}</h3>
    <p>{c["venue_count"]} live music venues · {c["show_count"]} upcoming shows</p>
    <span class="go">See venues →</span>
  </a>''' for slug, c in order)
    tot_v = sum(c["venue_count"] for c in cities.values())
...
{cards}
  ...
  <div class="cta">
    <h3>Looking for what's on, not where?</h3>
    <p>The Live page has every festival and tour across India, by date.</p>
    <a class="btn" href="/live/">See festivals &amp; tours →</a>
  </div>
</div>
{FOOT}'''
...
{cards}
  <div class="cta">
    ...
    <p>We help independent Indian artists get heard, get paid and get on stage.</p>
    <a class="btn" href="/programme/">The programme →</a>
    <a class="btn ghost" href="/live/">Festivals &amp; tours →</a>
  </div>
</div>
{FOOT}'''
        d = VDIR / slug
